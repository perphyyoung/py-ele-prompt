...
- **Prompt 管理** - 创建、编辑、删除、搜索 Prompt，支持标题重复检测和覆盖
- **标签系统** - 分离提示词标签和图像标签，支持标签筛选和管理
- **标签多选** - Shift 键多选标签进行组合筛选
- **特殊标签** - 标签管理界面显示特殊标签（收藏、多图/多引用、无图/未引用），始终显示在搜索框上方
- **收藏功能** - Prompt 和图像支持收藏/取消收藏，快速访问
- **排序功能** - Prompt 支持按更新时间、创建时间、标题排序；图像支持按上传时间、更新时间、文件名、尺寸排序
- **搜索功能** - 数据库层面的内容搜索，支持防抖优化，支持搜索图像备注
- **图像支持** - 为 Prompt 添加本地图像，支持图像管理和详情查看
- **图像导航** - 图像详情页支持左右切换、键盘导航、全屏查看
- **图像提示词预览** - 鼠标悬停图像卡片显示关联提示词
- **图像文件名编辑** - 图像查看界面支持编辑和保存文件名
- **多引用管理** - 支持一张图像关联多个提示词，图像详情显示所有关联提示词
