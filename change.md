...
- 图像详情界面点击"添加提示词"时，自动将当前图像预填充到参考图像区域
- 支持已保存图像对象的预览渲染，使用 `renderSavedImages()` 方法
- 新建提示词成功后，返回图像详情界面并同步更新左侧面板关联信息

### 详情界面切换优化

- 图像详情与提示词详情间切换采用 hide/show 机制，避免关闭重开
- `hide()` 方法临时重置 `isFromDetailJump`，允许提示词详情中的二级跳转
- `show()` 方法恢复禁止二级跳转状态
- 修复导航到新图像后"编辑提示词"按钮被禁用的问题

### 数据同步优化

- 图像和提示词的关联关系改变后，双方 `updated_at` 字段同时更新
- 优化事件机制，使用缓存保持数据最新状态
- 新建提示词成功后，更新关联图像的缓存数据

### 错误日志增强

- 增强错误日志记录，使用 `getDetailedStackTrace()` 提取详细堆栈
- 使用 👉 标记第一行（实际错误发生位置）
- 显示函数名、文件名、行号、列号，精确定位错误

### Bug 修复

- 修复新建提示词直接取消或关闭时的提醒问题
